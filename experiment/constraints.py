...
class DataTypeConstraint(Constraint):
    def satisfy(self, ctx: CandidateContext):
        valid = {"continuous_variable", "binary_class_variable", "multi_class_variable", "qualitative_variable"}
...
class ContinuousLogicConstraint(Constraint):
    def satisfy(self, ctx: CandidateContext):
...
class CategoricalLogicConstraint(Constraint):
    def satisfy(self, ctx: CandidateContext):
        if ctx.src_type in ["binary_class_variable", "multi_class_variable"] and ctx.tgt_type == ctx.src_type:
...
class ConstraintSolver:
    def __init__(self):
        ...
        self.constraints: List[Constraint] = [
            VisitConstraint(),
            DataTypeConstraint(),
            ContinuousLogicConstraint(),
            CategoricalLogicConstraint(),
            MismatchFallbackConstraint()
        ]
